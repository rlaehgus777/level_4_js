...
//복습) 함수, 매개변수, 리턴
//더하기, 빼기 계산기
function calcFunc(n1, n2, oper='+'){ //calcFunc라는 함수생성과 매개변수 3개 생성
    let num1 = n1; //변수생성:num1을 매개변수n1지정
    let num2 = n2;
    return oper == '+' ? num1+num2 : num1-num2; //oper의 값이 +라면 num1과num2를 더하고 +가 아니라면 num1과num2를 뺀다.
    //조건 ? 참 : 거짓;
    let total = num1 + num2;
    return total;
}
...
//할인율 계산기 (사용자 입력하는 가격은 다르고, 할인은 고정)
//계산법) (100 - 5) / 100 = 0.95
//판매가 * 0.95 = 할인판매가
// *0.95 = 5% 할인가 
// *0.9 = 10% 할인가 
...
console.log(discountFunc(29900));
console.log(discountFunc(1099000));
...
func2(); //익명함수이기에 변수안에서 생성한 익명함수 이전에 사용하면 안됨
...
plusBtn.addEventListener('click', ()=>{
    ...
    console.log(currentNum, typeof(currentNum)); //숫자로 변경된 점을 확인.
    
    currentNum > 19 ? alert('최대 주문 수량입니다.') : (()=>{
        let plusTotal = ++currentNum; //숫자로 변환한 변수를 1증가해서 담은 증가변수
        countNumSpan.dataset.count = plusTotal; //증가된 변수를 실제 data속성에 업데이트
        countNumSpan.textContent = plusTotal;//증가된 변수를 실제 화면에 출력
    })()
})
//-를 클릭하면 span값이 1씩 감소(익명함수)
